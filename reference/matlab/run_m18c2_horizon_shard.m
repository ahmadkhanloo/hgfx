function run_m18c2_horizon_shard(input_path, output_path)
%RUN_M18C2_HORIZON_SHARD Evaluate one frozen S7 shard in MATLAB.
%
...
assert(strcmp(payload.protocol, 'm18c2-trial-horizon-identifiability-1'));
...
for k = 1:numel(payload.parameter_cases)
    ...
    fprintf('M18C.2 MATLAB parameter %s success=%d termination=%s\n', c.case_id, r.success, r.termination_inferred);
end
...
for k = 1:numel(payload.model_cases)
    ...
    fprintf('M18C.2 MATLAB model %s selected=%s\n', c.case_id, mr.selected_model);
end
