function export_m18_d02_unitsq_current_argmax(output_plus_path, output_minus_path)
% Diagnostic-only D02 unit-square probes at the current post-reduction argmaxes.
% Evidence source: official run 34781625139 / artifact 10325069446.
%   plus:  parameter 2, Ridders step 8, h=1/1.2^7, trial 200 (1-based)
%   minus: parameter 2, Ridders step 9, h=1/1.2^8, trial 145 (1-based)
...
plus_payload = build_payload( ...
    ...
    1/(1.2^7), +1, 200, 'parameter_2_ridders_plus_step_8');
minus_payload = build_payload( ...
    r, u, sim.y, init, opt_idx, n_prcpars, n_obspars, x0, ...
    1/(1.2^8), -1, 145, 'parameter_2_ridders_minus_step_9');

write_payload(output_plus_path, plus_payload);
write_payload(output_minus_path, minus_payload);

fprintf('M18 D02 current unitsq probes: plus_trial=%d minus_trial=%d\n', ...
    plus_payload.trial_focus_1based, minus_payload.trial_focus_1based);
end

function payload = build_payload(r, u, responses, init, opt_idx, n_prcpars, n_obspars, x0, h, direction, trial_focus, sample_name)
sample_free = x0;
...
payload.trial_focus_1based = trial_focus;
payload.inputs = u;
