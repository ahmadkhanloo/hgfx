#!/usr/bin/env python3
"""Run HGFX on one frozen M18C.2 shard and compare to MATLAB raw fits."""
...
def _fit(model: str, y, u) -> dict[str, Any]:
    try:
        ...
        inv = np.asarray(fit.optimizer.inverse_hessian, dtype=np.float64)
        if inv.ndim != 2 or inv.shape[0] != inv.shape[1]:
            spectrum = []
            rank = 0
            cond = None
        else:
            inv_sym = 0.5 * (inv + inv.T)
            evals = np.linalg.eigvalsh(inv_sym)
            finite = evals[np.isfinite(evals)]
            spectrum = [float(x) for x in evals]
            rank = int(np.sum(np.abs(finite) > 1e-12))
            pos = finite[np.abs(finite) > 1e-12]
            cond = float(np.max(np.abs(pos)) / np.min(np.abs(pos))) if pos.size else None
        return {
            "model": model,
            "success": True,
            "error": None,
            ...
            "iterations": int(getattr(fit.optimizer, "n_iter", getattr(fit.optimizer, "iterations", 0)) or 0),
            "reset_count": int(getattr(fit.optimizer, "reset_count", 0) or 0),
            "converged": fit.optimizer.termination in {"tol_arg", "tol_grad"},
            "inverse_hessian_eigenvalues": spectrum,
            "inverse_hessian_rank": rank,
            "inverse_hessian_condition": cond,
        }
...
def main(input_path: str, matlab_path: str, output_path: str) -> int:
    ...
    if manifest["protocol"] != PROTOCOL or matlab_raw["protocol"] != PROTOCOL:
        raise ValueError("M18C.2 protocol mismatch")
