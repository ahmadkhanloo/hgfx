#!/usr/bin/env python3
"""Classify frozen MATLAB one-spacing start sensitivity for official D08."""
...
def main(reference_path: Path, output_path: Path) -> int:
    ...
    if reference.get("case_id") != "D08_fit" or int(reference.get("seed")) != 123456789:
        raise ValueError("Frozen official D08 contract mismatch")
    if reference.get("model") != "uhgf" or reference.get("observation") != "gaussian_obs":
        raise ValueError("Frozen official D08 model contract mismatch")
    ...
    if free_init.size != 7 or free_indices.tolist() != EXPECTED_FREE_FULL_INDICES:
        raise ValueError("Frozen official D08 free-parameter contract mismatch")
    ...
    material = [r["label"] for r in results if not r["final_within_existing_gate"]]
    result = {
        "protocol": PROTOCOL,
        ...
        "decision_note": (
            "A positive result is exact official-D08 MATLAB optimizer sensitivity evidence only. "
            "It does not rewrite the direct failure or independently confer PASS/REFERENCE_LIMITATION_MATCH."
        ),
    }
