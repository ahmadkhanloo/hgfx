#!/usr/bin/env python3
"""Classify frozen MATLAB-only one-spacing start sensitivity for D08."""
...
def main(reference_path: Path, output_path: Path) -> int:
    ...
    if reference.get("case_id") != "D08_fit" or int(reference.get("seed")) != 314159265:
        raise ValueError("Frozen D08 contract mismatch")
    if reference.get("model") != "uhgf" or reference.get("observation") != "gaussian_obs":
        raise ValueError("Frozen D08 model contract mismatch")
    ...
    if free_init.size != 7 or free_indices.tolist() != EXPECTED_FREE_FULL_INDICES:
        raise ValueError("Frozen D08 free-parameter contract mismatch")
        ...
        results.append(
            {
                ...
                "reset_indices_1based": [int(x) for x in _array(item["iter_rst"]).reshape(-1)]
                if np.asarray(_array(item["iter_rst"])).size
                else [],
    ...
    result = {
        ...
        "decision_note": (
            "A positive result is exact-scope MATLAB optimizer sensitivity evidence only. "
            "It does not rewrite the failed D08 prospective holdout or independently confer PASS/REFERENCE_LIMITATION_MATCH."
        ),
    }
