...
from check_m18_demo_uhgf_ar1 import _array
...
def arr(value: Any) -> np.ndarray:
    """Decode the repository's ieee-strings-v1 MATLAB JSON representation."""
    return _array(value)
...
def compare(label: str, actual: Any, expected: Any):
    ...
    return {
        ...
        "pass": False,
        "index": list(idx),
        "hgfx": float(a[idx]) if idx else float(a),
        "matlab": float(e[idx]) if idx else float(e),
        "abs_diff": float(abs((a[idx] if idx else a) - (e[idx] if idx else e))),
    }
...
def main(reference_path: Path, output_path: Path) -> int:
    ...
    if ref.get("protocol") != PROTOCOL or ref.get("reference_commit") != REFERENCE_COMMIT:
        raise ValueError("Frozen D09 protocol/reference mismatch")
    if [int(x) for x in ref.get("seeds", [])] != [123, 456]:
        raise ValueError("Frozen D09 seeds mismatch")

    inputs = arr(ref["inputs"]).reshape(-1)
    results = []
    failures = []
    for case in ref.get("cases", []):
        ...
        checks = [
            ...
            compare("p_prc", result.perceptual_parameters, case["p_prc"]),
            compare(
                "p_obs_trans",
                result.observation_transformed_parameters,
                case["p_obs_trans"],
            ),
            compare("p_obs", result.observation_parameters, case["p_obs"]),
        ]
        ...
        for field, expected in expected_traj.items():
            if field not in result.trajectory:
                checks.append(
                    {"label": f"traj.{field}", "pass": False, "reason": "missing_field"}
                )
            else:
                checks.append(compare(f"traj.{field}", result.trajectory[field], expected))

        matlab_ignored = tuple(
            int(x) - 1 for x in arr(case.get("ignored_matlab", [])).reshape(-1)
        )
        matlab_seed = int(arr(case["c_sim_seed"]).reshape(-1)[0])
        semantics_pass = (
            result.ignored_trials == matlab_ignored
            and result.seed == seed
            and matlab_seed == seed
        )
        checks.append(
            {
                "label": "result_semantics",
                "pass": semantics_pass,
                "hgfx_ignored": list(result.ignored_trials),
                "matlab_ignored": list(matlab_ignored),
                "seed": seed,
            }
        )

        case_pass = all(bool(check["pass"]) for check in checks)
        if not case_pass:
            failures.extend(
                [{"seed": seed, **check} for check in checks if not check["pass"]]
            )
        results.append(
            {
                "seed": seed,
                "pass": case_pass,
                "checks": checks,
                "matlab_response_shape": list(arr(case["responses"]).shape),
                "response_equality_gated": False,
            }
        )
    ...
    out = {
        ...
        "failures": failures,
        "rng_note": (
            "Cross-runtime parameter sampling uses exact MATLAB-exported standard-normal "
            "drivers; MATLAB response RNG bytes are not equality-gated."
        ),
    }
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(out, indent=2) + "\n")
    print(
        json.dumps({"classification": classification, "failures": failures[:3]}),
        flush=True,
    )
