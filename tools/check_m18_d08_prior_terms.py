...
def main(reference_path: Path, output_path: Path) -> int:
    ...
    parameters = full[indices]
    means = prc.priormus[indices]
    variances = prc.priorsas[indices]
    ...
    scalar_norm = np.asarray(
        [-0.5 * math.log(8.0 * math.atan(1.0) * float(v)) for v in variances],
        dtype=np.float64,
    )
    scalar_quad = np.asarray(
        [-0.5 * ((float(p) - float(m)) ** 2) / float(v) for p, m, v in zip(parameters, means, variances, strict=True)],
        dtype=np.float64,
    )
    ...
    result = {
        ...
        "python_sequential_sum_matlab_terms": scalar_cmp(
            np.float64(sum(float(v) for v in matlab_terms)), matlab_total
        ),
