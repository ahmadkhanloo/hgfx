...
PERCEPTUAL_RULES = (
    ...
    Rule("hgf_binary_mab", "binary MAB", "DONE", "M12", "multi-arm state tensors and reward-coded inputs are outside the frozen core compatibility slice"),
    Rule("hgf_ar1_mab", "continuous AR1 MAB", "DONE", "M12", "multi-arm continuous state tensors need a dedicated compatibility API"),
    ...
    Rule("hgf_ar1", "continuous AR1 HGF", "DONE", "M12", "continuous AR1 was not required by the P1 binary specialized gate and has a distinct parameter layout"),
    ...
    Rule("hgf_jget", "JGET HGF", "DONE", "M12", "specialized JGET state recursion and parameterization require a dedicated scientific fixture"),
    ...
    Rule("hgf_categorical_norm", "categorical normalized HGF", "DONE", "M12", "multinomial tensor recursion is isolated from the binary/continuous compatibility core"),
    ...
    Rule("hgf_whatworld", "WhatWorld HGF", "DONE", "M12", "high-dimensional transition/world latent-state semantics need dedicated fixtures"),
    Rule("hgf_whichworld", "WhichWorld HGF", "DONE", "M12", "world-mixture latent-state semantics need dedicated fixtures"),
    ...
    Rule("tapas_hhmm", "HHMM", "DONE", "M12", "frozen file declares htapas_hmm inside tapas_hhmm.m and uses tree flattening semantics requiring source repair/fixture before port"),
    Rule("bayes_optimal", "Bayes-optimal auxiliary families", "DONE", "M12", "stored in perceptual/ but implement auxiliary likelihoods rather than a standalone latent-state recursion"),
    Rule("rs_", "response-surprise auxiliary models", "DONE", "M12", "auxiliary surprise/belief/precision models are retained as reference-only analysis models"),
    Rule("squared_pe", "squared prediction-error auxiliary", "DONE", "M12", "auxiliary deterministic analysis model; not part of the compatibility forward core"),
...
OBSERVATION_RULES = (
    Rule("condhalluc_obs", "conditional hallucination observations", "DONE", "M12", "P2 family coupled to specialized conditional-hallucination perceptual integrations"),
    Rule("logrt_linear_whatworld", "WhatWorld logRT", "DONE", "M12", "depends on reference-only WhatWorld state layout"),
    Rule("softmax_mu3_wld", "world mu3 softmax", "DONE", "M12", "depends on reference-only world state layout"),
    Rule("softmax_wld", "world softmax", "DONE", "M12", "depends on reference-only world state layout"),
