...
- No remediation, merge, tag, `workflow_dispatch`, P6A freeze or pull-request was performed. On 2026-09-26 the owner authorised publishing this record as repository evidence by pushing the review branch alone: `origin/p8-review/b66f294`, carrying review-only commits `4370342` (this report), `f390e23` (checklist) and the commit that records the wording of this paragraph (its own SHA is deliberately not quoted — a commit cannot name itself; read the tip with `git ls-remote origin p8-review/b66f294`). `main`, `p8-final-candidate-20260921` and `p8-remediation-20260926` were left untouched, so the reviewed candidate and its lock chain are unmodified by publication. No manuscript, supplement or review text was sent to any third-party service, and `gh` run-metadata verification was blocked by the local-only handling policy and was **not** worked around; the six claimed run IDs therefore remain documentary.
