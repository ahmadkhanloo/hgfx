...
| P2 core equivalence tables reproduce deterministically from frozen v1 evidence | READY / P2 PASS | `paper/scripts/generate_p2_tables.py`; seven committed `paper/tables/*.md` outputs; `paper/tables/p2_tables_manifest.json`; focused CI run `35231582282` on `332d248a7ba51ae4ba5d25a387d78bfbc71855b0` |
| P2 table provenance is checksum-traceable | READY / P2 PASS | committed P2 manifest records SHA-256 for all required evidence inputs and all generated table outputs; CI regenerates and checks zero Markdown diff |
...
| P6A draft evidence inventory is deterministic and cross-platform | READY / P6A-1 PASS / NOT FROZEN | `paper/reproducibility/p6a_paper_evidence_manifest.json`; the manifest `file_count` is authoritative for the current draft inventory; canonical Git-byte SHA-256; deterministic PNG/PDF outputs; P6A-1 PR #61 / run `35333572515` PASS; P6A-2 PR #63 / run `35335397062` PASS |
...
| CPU compatibility and JAX CPU backend preserve tested outputs | READY / POST-FIX REMEASURED | `gpu_validation_results/m18_s9_cpu_postfix_revalidation.json`; Actions run `36255345703`; source `e153a3c0`; max fit-objective gap `0.006783711260709424` vs frozen `0.1` criterion; repeated raw outputs byte-identical |
| Physical NVIDIA GPU path preserves tested final objective values | READY / SCOPED / RETAINED | pre-release source `07b45a569e04e8e71244c5310dd2cc53dbb2b0ec`; two Tesla T4 devices visible, required fitting cells on `cuda:0`; JAX device placement + `nvidia-smi -L` enumeration; `PASS_PHYSICAL_GPU_APPLICABILITY`; max gap `1.4210854715202004e-14` vs `1e-7` |
