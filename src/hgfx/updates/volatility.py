...
def hgf_volatility_update(
    ...
    with np.errstate(over="ignore", under="ignore", divide="ignore", invalid="ignore"):
        v_lower = t * matlab_exp_scalar(ka * mu_prev + om)
        ...
        if update_type == "ehgf":
            ...
            vv = t * matlab_exp_scalar(ka * mu + om)
        ...
        if update_type == "uhgf":
            v_lower = t * matlab_exp_scalar(ka * muhat + om)
            if np.isinf(v_lower):
            ...
            blend = np.float64(1.0) / (
                np.float64(1.0) + matlab_exp_scalar(i1 - i2)
            )
