...
| uHGF → AR(1) official workflow | PASS |
...
The trial-horizon study (section 2.6) is executing. Until all 24 shards are classified, this manuscript does not claim generally strong parameter recovery for HGFX or for the MATLAB reference.
...
| Physical NVIDIA GPU (2× Tesla T4) | PASS_PHYSICAL_GPU_APPLICABILITY | max \|Δ objective\| = 1.42e-14; criterion 1e-7 |
...
Current limitations: (i) compatibility is scoped to HGF Toolbox 8.2.0, not future upstream versions; (ii) D02/D08 are matched reference limitations, not scientific recovery success; (iii) parameter identifiability is weaker than paired model selection, and the trial-horizon classification is not yet available; (iv) GPU evidence is T4 applicability, not throughput; (v) the pyhgf result is one mapped cell, not package-wide equivalence.
...
Bibliography entries are in `paper/references.bib` (Frontiers author–date style on submission).
